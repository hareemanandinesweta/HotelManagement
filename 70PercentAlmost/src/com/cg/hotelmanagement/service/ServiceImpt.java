...
public class ServiceImpt implements ServiceInteface {
	...
	public boolean logIn(Customer C, String id, HashMap<Customer, String> CustMap, String pass) {
		...
		if (C.getCustId().equals(id) && CustMap.containsValue(pass)) {
...
//					customer1.setBillAmount(cost);
	...
	public void cancelRoom(ArrayList<Room> roomList, Customer cst) {
		...
		ArrayList<Integer> roomsIdList = (ArrayList<Integer>) roomList.stream().filter(r -> r.isIsbooked())
				.filter(r -> r.getCustomer().getCustId().equalsIgnoreCase(cst.getCustId())).map(r -> r.getRoomid())
		...
		for (Integer roomNo : roomsIdList) {
			for (Room r : roomList) {
	...
	public void checkOut(String custID, ArrayList<Room> roomList) {
		{
			...
			roomList.stream().filter(r -> r.isIsbooked())
					.filter(c -> c.getCustomer().getCustId().equalsIgnoreCase(custID)).forEach(r -> {
						r.setIsbooked(false);
						r.setCustomer(null);
					});
